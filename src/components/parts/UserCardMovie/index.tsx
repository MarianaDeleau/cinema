        ...
        if (item.media_type === media_type) {
          return (
            <Card sx={{ width: 200, margin: 2 }} className="card_movie">
              <CardActionArea
                onClick={() => navigate(`/detail?id=${item.idDB}`)}
              >
                <CardMedia
                  component="img"
                  height="300"
              ...
              <CardActions sx={{ justifyContent: "center" }}>
                {userLogged?.role === "admin" && IsMovieInDB(item.id) && (
                  <CardActions sx={{ justifyContent: "center" }}>
                    <Button
                      size="small"
                      sx={{ backgroundColor: "black", width: 120 }}
                      type="submit"
                      onClick={() => {
                        deleteMoviesFromDB(item.id);
                      }}
                    >
                      REMOVER
                    </Button>
                ...
                {userLogged?.role === "user" &&
                  ((!isItemViewed(item.idDB) && (
                    <Button
                      size="medium"
                      sx={{ backgroundColor: "red", width: 120 }}
                      type="submit"
                      onClick={() => {
                        addItemToList(userLogged, item.idDB);
                      }}
                    >
                      <EyeSlashFill />
                    </Button>
                    ...
                    (isItemViewed(item.idDB) && (
                      <Button
                        size="medium"
                        sx={{ backgroundColor: "gray", width: 120 }}
                        type="submit"
                        onClick={() => {
                          removeItemFromList(userLogged, item.idDB);
                        }}
                      >
                        <EyeFill />
                      </Button>
